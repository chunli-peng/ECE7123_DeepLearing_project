...
class Segmentator(nn.Module):
	def __init__(self, ARCH, nclasses, path=None, path_append="", strict=False):
		...
		bboneModule = SourceFileLoader("bboneModule", r'../../' + '/backbones/' +
									   self.ARCH["backbone"]["name"] + '.py').load_module()
		...
		decoderModule = SourceFileLoader("decoderModule", r'../../' + '/tasks/semantic/decoders/' +
										 self.ARCH["decoder"]["name"] + '.py').load_module()
		self.decoder = decoderModule.Decoder(params=self.ARCH["decoder"],
											 stub_skips=stub_skips,
											 network_layers=self.ARCH['backbone']['extra']['layers'],
