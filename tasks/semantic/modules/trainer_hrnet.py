...
class Trainer():
	def __init__(self, ARCH, DATA, datadir, logdir, testdata, path=None):
		...
		self.lr_group_names = ['hrnet_model_lr']
	...
	def train(self):
		...
		for epoch in range(self.ARCH["train"]["max_epochs"]):
			...
			if iou > best_train_iou:
				...
				torch.save(self.model_single.state_dict(), self.log + "/hrnet_train")
	...
	def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
		...
		for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(
			...
			# compute output
			######################################################################
			# hrnet: not using CRF, so not using proj_mask, only y = model(x)
			######################################################################
			output = model(in_vol)
