...
class Trainer():
	def __init__(self, ARCH, DATA, datadir, logdir, testdata, path=None):
		...
		parserPath = r'../../tasks/semantic/dataset/kitti/parser.py'
		...
		with torch.no_grad():
			self.model = ConvNeXt(in_chans=5, dims=[96, 192, 384, 768])
		...
		self.lr_group_names = ['resnest_model_lr']
		self.train_dicts = [{'params': self.model_single.parameters()}]
	...
	def train(self):
		...
		for epoch in range(self.ARCH["train"]["max_epochs"]):
			...
			if iou > best_train_iou:
				...
				torch.save(self.model_single.state_dict(), self.log + "/resnest_train")
			...
			if epoch % self.ARCH["train"]["report_epoch"] == 0:
				...
				if iou > best_val_iou:
					...
					torch.save(self.model_single.state_dict(), self.log + "/resnest")
