...
class ConvNeXt(nn.Module):
	...
	def __init__(self, in_chans=3, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768],
		...
		stem = nn.Sequential(
			nn.Conv2d(in_chans, dims[0], kernel_size=3, stride=[1, 4], padding=[1, 0]),
		...
		for i in range(3):
			downsample_layer = nn.Sequential(
				...
				nn.Conv2d(dims[i], dims[i + 1], kernel_size=3, stride=[1, 2], padding=[1, 1]),
			)
		...
		for i_layer in range(4):
			...
			self.unconv.append(nn.Sequential(
				nn.ConvTranspose2d(self.dims[0], 32, kernel_size=[3, 4], stride=[1, 4], padding=[1, 0]),
				LayerNorm(32, eps=1e-6, data_format="channels_first"),
				nn.GELU(),
				Block(dim=32),
			))
